...
while cap.isOpened():
    ...
    for label, position in displayed_hand_positions.items():
        center_x = int(position[0] * display_frame.shape[1])
        center_y = int(position[1] * display_frame.shape[0])
        marker_color = (0, 180, 255)
        cv2.circle(display_frame, (center_x, center_y), 8, marker_color, -1)
        cv2.putText(
            display_frame,
            label,
