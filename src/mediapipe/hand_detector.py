...
#Set recognizer based on Instrument
if INSTRUMENT == 0:
...
# Initialize a frame counter for timestamps
frame_counter = 0
...
while cap.isOpened():
    ...
    if INSTRUMENT == 0: # Theremin uses HandLandmarker in VIDEO mode
        # Use the frame counter as the timestamp
        recognition_result = recognizer.detect_for_video(image, frame_counter)
    else: # Drums use GestureRecognizer
    ...
    #draw frame , so that text or circles are not drawn on the raw camera feed, but on the displayed frame
